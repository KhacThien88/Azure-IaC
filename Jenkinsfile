...
stage('Install Ansible and playbook') {
    steps {
        ...
        sshCommand(remote: vm1, command: """
                ...
                ansible-playbook -i ~/kubespray/inventory/mycluster/inventory.ini --become --become-user=root --ask-become-pass ~/kubespray/cluster.yml || { echo 'ansible-playbook failed!'; exit 1; }
            """)
